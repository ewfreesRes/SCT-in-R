...
For a given set of pre-exercise code, hint, soluton, sct, and sample code, all of the "ex" should be the same. This way DataCamp Lite knows to combine them into a single exercise, as opposed to making each one belong to its own exercise.
## Solution
...
As a note, writing exercises in Datacamp works exactly the same as DataCamp Lite, so anything you find in regards to DataCamp can more than likely be applied to DataCamp Lite